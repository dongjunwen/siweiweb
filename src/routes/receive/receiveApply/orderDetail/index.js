...
class AdvancedSearchForm extends React.Component {
  ...
  handleSubmit = () => {
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      const values = {
        ...fieldsValue,
        recvDate: fieldsValue.recvDate && fieldsValue.recvDate.format('YYYY-MM-DD'),
      };
  ...
  render() {
    const {form: {getFieldDecorator}, swReceiveBaseResultVo, readOnly} = this.props;
...
class CreateOrderPage extends React.Component {
  ...
  handleSubmit = (formValue) => {
    formValue.recvNo = this.props.orderDetail.swReceiveBaseResultVo.recvNo;
    request({
      url: `${config.APIV0}/api/receive`,
      method: 'PUT',
      data: {
        swReceiveBaseModiVo: formValue,
